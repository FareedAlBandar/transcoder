...
// Probe ...
func (t *Transcoder) Probe() (map[string]interface{}, error) {
	_, err := t.getMetadata()
	if err != nil {
		return nil, err
	}
	...
	toggle := 0
	if t.metadata.Streams[0].CodecType != "video" {
		toggle = 1
	}

	video := t.metadata.Streams[toggle]
	audio := t.metadata.Streams[1-toggle]
